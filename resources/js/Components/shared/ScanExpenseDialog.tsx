...
type Draft = { token: string; supplier_name?: string | null; supplier_tax_id?: string | null; invoice_number?: string | null; date?: string | null; total_amount?: number | null; support_name?: string };
...
export function ScanExpenseDialog({ label = 'Escanear gasto', className = '' }: { label?: string; className?: string }) {
    ...
    return <>
        ...
        <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-xl border-white/10 bg-[#0D0A0A]"><DialogHeader><DialogTitle className="flex items-center gap-2"><ScanLine className="h-5 w-5 text-[#D4AF37]" />Escanear gasto</DialogTitle><DialogDescription>Sube una factura, recibo o comprobante y completaremos los datos automáticamente.</DialogDescription></DialogHeader>
            ...
            {draft && !loading && <div className="space-y-4"><div className="flex items-center gap-2 text-emerald-300"><CheckCircle2 className="h-5 w-5" />Documento analizado</div><div className="grid gap-3 rounded-lg border border-white/10 bg-white/[0.03] p-4 sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">Proveedor</p><p>{draft.supplier_name ?? 'No identificado'}</p></div><div><p className="text-xs text-muted-foreground">NIT</p><p>{draft.supplier_tax_id ?? 'No identificado'}</p></div><div><p className="text-xs text-muted-foreground">Factura</p><p>{draft.invoice_number ?? 'No identificada'}</p></div><div><p className="text-xs text-muted-foreground">Fecha</p><p>{draft.date ?? 'No identificada'}</p></div><div className="sm:col-span-2"><p className="text-xs text-muted-foreground">Total</p><p className="text-xl font-semibold text-[#D4AF37]">{draft.total_amount != null ? new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(Number(draft.total_amount)) : 'No identificado'}</p></div></div><Button className="w-full" onClick={continueToForm}>Continuar con el registro</Button></div>}
